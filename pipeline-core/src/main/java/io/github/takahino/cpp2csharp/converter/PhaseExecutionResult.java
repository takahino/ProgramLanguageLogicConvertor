 ...
 *
 * @param tokenNodes
 *            更新済みトークンノード列。テキスト系フェーズ（COMBY 等）では null を返し、 前フェーズのトークンノードが引き継がれる。
 * @param commentsBeforeToken
 *            更新済みコメントマップ
 * @param code
 *            フェーズ適用後のコード文字列。トークン系フェーズは {@code buildOutput()} で生成。
 * @param snapshots
 *            このフェーズが生成したスナップショット（0 件以上）
 * @param logs
 *            このフェーズが生成した適用ログ（0 件以上）
 * @param unitOutputDumps
 *            MAIN フェーズ全ユニットの変換後テキスト（他フェーズでは空リスト）
 * @param unitLabels
 *            各ユニットの種別（他フェーズでは空リスト）
 */
public record PhaseExecutionResult(List<AstNode> tokenNodes, Map<Integer, List<String>> commentsBeforeToken,
		String code, List<PhaseSnapshot> snapshots, List<PhaseTransformLog> logs, List<String> unitOutputDumps,
