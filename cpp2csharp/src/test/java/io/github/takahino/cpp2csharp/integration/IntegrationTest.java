 ...
 *
 * <p>
 * {@code src/test/resources/cpp/} 配下の C++ ファイルを読み込み、 変換ルールを適用した結果を検証する。
 * </p>
 *
 * <h2>テスト対象ファイル</h2>
 * <ul>
...
class IntegrationTest {
	 ...
	 *
	 * <p>
	 * コメント保持機能により出力にコメントが含まれる場合でも、 コード本体部分のみに対してアサーションを行うために使用する。
	 * </p>
	 *
	 * @param code
	 *            変換後の C# コード文字列
	...
	void testPassLimitReached() throws IOException {
		...
		assertThat(result.getAppliedTransforms()).as("10 パスで 10 件のみ変換されること").hasSize(10);
		assertThat(result.getTransformErrors()).as("パス上限到達エラーが記録されること").singleElement().satisfies(
				e -> assertThat(e.getErrorType()).isEqualTo(Transformer.TransformError.ErrorType.PASS_LIMIT_REACHED));
		assertThat(result.getCsCode()).as("未変換の INT が残ること").contains("INT ");
	}
	...
	class MessagesFileTests {
		...
		void testSimpleError() throws IOException {
			...
			assertThat(csCode).as("AfxMessageBox が MessageBox.Show に変換されること").contains("MessageBox.Show")
					.as("MB_ICONERROR が MessageBoxIcon.Error に変換されること").contains("MessageBoxIcon.Error")
					.as("MB_OK が MessageBoxButtons.OK に変換されること").contains("MessageBoxButtons.OK");

			assertThat(stripComments(csCode)).as("AfxMessageBox というトークンが残らないこと (コメント除く)")
	...
	class CompoundFileTests {
		...
		void testFullCompoundFile() throws IOException {
			...
			assertThat(result.getTransformErrors()).as("曖昧マッチエラーが発生しないこと").isEmpty();
	...
	class ErrorCaseTests {
		...
		void testUnmatchedCodePassthrough() throws IOException {
			...
			List<ConversionRule> rules = loader.loadFromString(
					"from: AfxMessageBox ( ABSTRACT_PARAM00 )\nto: MessageBox.Show(ABSTRACT_PARAM00)", "test.rule");

			ConversionResult result = converter.convertSource(cpp, rules);
			String csCode = result.getCsCode();

			assertThat(csCode).as("AfxMessageBox がないコードは変換されないこと").doesNotContain("MessageBox.Show");
			assertThat(csCode).as("変換対象外のトークンはそのまま残ること").contains("add").contains("return");
		}
