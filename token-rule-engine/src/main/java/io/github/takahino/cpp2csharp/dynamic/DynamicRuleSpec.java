 ...
 *
 * <p>
 * トークンストリームからパターンに一致する値を収集し、 その値をテンプレートに埋め込んで ConversionRule を動的に生成する。
 * </p>
 *
 ...
 *
 * <p>
 * 上記の場合、トークンストリームから `SomeClass ::` パターンを探し、 各クラス名について `void SomeClass :: AP00 (
 * AP01 )` → `private void AP00(AP01)` の ConversionRule を生成する。
 * </p>
 *
 * @param sourceFile
 *            定義元 .drule ファイル名
 * @param collectPattern
 *            収集パターン（ABSTRACT_PARAM00 が収集対象の値を捕捉する）
 * @param templates
 *            from/to テンプレートのリスト（COLLECTED が収集値に置換される）
 */
public record DynamicRuleSpec(String sourceFile, List<ConversionToken> collectPattern, List<FromToTemplate> templates) {

	/**
	 * from/to テンプレートのペア。
	 *
	 * @param fromTemplate
	 *            from パターンのテンプレート文字列（COLLECTED プレースホルダを含む）
	 * @param toTemplate
	 *            to テンプレート文字列（COLLECTED プレースホルダを含む場合がある）
	 */
