...
/**
 * COMBYフェーズの1変換ルールを表すクラス。 from パターン（:[name] ホール形式）と to テンプレート文字列で構成される。
 */
public final class CombyRule {

	private final String sourceFile;
	private final String fromPattern;
	private final String toTemplate;
	private final List<CombyTestCase> testCases;

	public CombyRule(String sourceFile, String fromPattern, String toTemplate, List<CombyTestCase> testCases) {
		this.sourceFile = Objects.requireNonNull(sourceFile);
		this.fromPattern = Objects.requireNonNull(fromPattern);
		this.toTemplate = Objects.requireNonNull(toTemplate);
		this.testCases = List.copyOf(testCases != null ? testCases : List.of());
	}
	...
	public List<CombyTestCase> getTestCases() {
		return testCases;
	}

	@Override
	public String toString() {
		return "CombyRule{from='" + fromPattern + "', file='" + sourceFile + "'}";
	}
}
