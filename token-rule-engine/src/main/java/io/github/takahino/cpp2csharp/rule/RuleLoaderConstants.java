...
/**
 * ルールローダー間で共有する定数を保持するユーティリティクラス。
 */
public final class RuleLoaderConstants {

	private RuleLoaderConstants() {
	}

	/** フェーズディレクトリ名のパターン: [01]_名前, [02]_名前 等 */
	public static final Pattern PHASE_DIR_PATTERN = Pattern.compile("^\\[(\\d{2})\\]_(.+)$");

	/**
	 * MAINサブフェーズのフォルダ名末尾に付けると、そのフェーズ後に再トークン化を行う。
	 *
	 * <p>
	 * デフォルト（サフィックスなし）は再トークン化を行わない（安全側）。 後続サブフェーズが合成ノード内の個々のトークンを認識する必要がある場合に明示指定する。
	 * </p>
	 *
	 * <p>
	 * 例: {@code [01]_ブロックコメント+after_retok} → このフェーズ後に再トークン化する
	 * </p>
	 */
	public static final String AFTER_RETOK_SUFFIX = "+after_retok";
}
