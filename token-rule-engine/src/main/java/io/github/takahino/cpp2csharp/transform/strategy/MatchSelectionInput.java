 ...
 * マッチ選択戦略に渡す入力 DTO。
 */
public record MatchSelectionInput(List<MatchResult> allMatches, List<String> tokens, MatchSelectionContext context) {

	public MatchSelectionInput {
		if (allMatches == null)
			throw new IllegalArgumentException("allMatches は null にできません");
		if (tokens == null)
			throw new IllegalArgumentException("tokens は null にできません");
	}
}
