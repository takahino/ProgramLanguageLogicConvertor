...
/**
 * MultiReplaceRule のスコープを表す列挙型。
 *
 * <ul>
 * <li>{@link #NONE} - スコープなし（ファイル全体が対象）</li>
 * <li>{@link #BLOCK} - ブロックスコープ（{@code { }} で囲まれたブロック内が対象）</li>
 * </ul>
 */
public enum MRuleScope {
	/** スコープなし（ファイル全体が対象） */
	NONE,
	/** ブロックスコープ（{@code { }} で囲まれたブロック内が対象） */
	BLOCK
}
