...
/**
 * MAINフェーズの1サブフェーズを表す record。
 *
 * <p>
 * {@link ConversionRuleLoader} が {@code [NN]_*} ディレクトリから生成し、 {@code MainPhase}
 * が消費する。
 * </p>
 *
 * <h2>retokenizeAfter の意図</h2>
 *
 * <p>
 * <strong>再トークン化をスキップする理由（デフォルト: {@code retokenizeAfter=false}）:</strong><br>
 * splice による置換結果は1つの合成ノードとしてトークン列に格納される。 再トークン化すると、前フェーズで C# として確定した箇所が再び個々の
 * C++ トークンに分解され、 後続サブフェーズのルールが意図せずマッチするリスクがある。
 * このリスクを避けるため、再トークン化しないことをデフォルトとする。
 * </p>
 *
 * <p>
 * <strong>再トークン化が必要な場合（{@code retokenizeAfter=true}）:</strong><br>
 * 後続サブフェーズが合成ノード内の個々のトークン（例: {@code "Math"}, {@code "."}, {@code "Sin"}）を
 * パターンとして認識させたい場合は、合成ノードを分解する必要がある。 フォルダ名に {@code "+after_retok"}
 * サフィックスを付けることで明示的に有効化できる。
 * </p>
 *
 * @param dirName
 *            フォルダ名（ロギング・デバッグ用）
 * @param rules
 *            このサブフェーズの変換ルールリスト
 * @param retokenizeAfter
 *            このフェーズ後に再トークン化を行うか
 */
public record MainPhaseSubPhase(String dirName, List<ConversionRule> rules, boolean retokenizeAfter) {
}
