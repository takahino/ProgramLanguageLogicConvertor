 ...
 *
 * <p>
 * 全変換フェーズ完了後に、フィルタ無視で再度 matchAll した結果のうち、 通常選択では未採用だった候補をレポート用に保持する。
 * </p>
 *
 * <p>
 * <strong>A1 ニアミス検出のため</strong>、{@code reasonCategory} が {@code "near_miss"}
 * の場合に 「どのパターントークンで不一致だったか」を示す3フィールドを追加した。
 * これにより「あと1トークンだけ違うルール」をレポートで可視化でき、ルール修正コストを大幅に削減できる。 非ニアミス時は null
 * を格納することで後方互換を維持する。
 * </p>
 *
 * @param lineNumber
 ...
 *            from パターン（文字列表現）
 * @param expandedTo
 *            想定 to テンプレート展開結果（もし変換されたら何になるか）
 * @param matchedText
 *            最終トークン列上でマッチした範囲の文字列
 * @param lineContent
 *            該当行の内容（スペース区切りトークン列）
 * @param reasonCategory
 *            分類（still_matchable_after_all_phases / near_miss 等）
 * @param mismatchPatternIndex
 *            A1 ニアミス時: マッチが失敗したパターンインデックス（null = 非ニアミス）
 * @param expectedToken
 *            A1 ニアミス時: パターンが期待したトークン（null = 非ニアミス）
 * @param actualToken
 *            A1 ニアミス時: 実際に存在したトークン（null = 不明）
 */
public record DiagnosticCandidate(int lineNumber, String ruleSource, String ruleFrom, String expandedTo,
		String matchedText, String lineContent, String reasonCategory, Integer mismatchPatternIndex,
		String expectedToken, String actualToken) {

	/**
	 * 後方互換コンストラクタ。A1 ニアミスフィールドは null で初期化する。
	 */
	public DiagnosticCandidate(int lineNumber, String ruleSource, String ruleFrom, String expandedTo,
			String matchedText, String lineContent, String reasonCategory) {
		this(lineNumber, ruleSource, ruleFrom, expandedTo, matchedText, lineContent, reasonCategory, null, null, null);
	}
}
