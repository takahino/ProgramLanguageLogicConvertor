...
/**
 * マッチ選択戦略が曖昧マッチを報告するためのコンテキスト。
 *
 * <p>
 * 戦略は副作用を持たず、曖昧マッチ時はこのコンテキストを通じて 呼び出し元（Transformer）に報告する。
 * </p>
 */
public interface MatchSelectionContext {
	 ...
	 *            曖昧なマッチ候補のリスト
	 * @param contextTokens
	 *            曖昧マッチが発生したトークン列の文字列表現（ログ用）
	 */
	void reportAmbiguous(List<MatchResult> ambiguousMatches, String contextTokens);
}
